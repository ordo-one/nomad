...
func NewMockCSIManager(t *testing.T, eventer TriggerNodeEvent, plugin csi.CSIPlugin, pluginID, externalID string, requiresStaging bool) *MockCSIManager {
	vm := newVolumeManager(
		testlog.HCLogger(t),
		eventer,
		plugin,
		t.TempDir(), // rootDir,
		t.TempDir(), //	containerRootDir ,
		requiresStaging,
		externalID)
